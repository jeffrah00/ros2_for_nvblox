...
class CustomStereoDepthNode(Node):
    ...
    def _trt_infer(self, left_np: np.ndarray, right_np: np.ndarray):
        ...
        # Squeeze is a view of the pinned buffer; only cast when the engine
        # emits something other than float32. Downstream np.divide(out=depth)
        # reads the pinned buffer before the next H2D overwrites it (stream
        # is synchronized above; this callback is single-threaded).
        disp = np.squeeze(out_buffers[0]['host'])
        occ = np.squeeze(out_buffers[1]['host']) if len(out_buffers) > 1 else None
        conf = np.squeeze(out_buffers[2]['host']) if len(out_buffers) > 2 else None
        if disp.dtype != np.float32: disp = disp.astype(np.float32)
        if occ is not None and occ.dtype != np.float32: occ = occ.astype(np.float32)
        if conf is not None and conf.dtype != np.float32: conf = conf.astype(np.float32)
        t_out = time.perf_counter()
        self._last_infer_timing = {
            'conv': (t_conv - t0) * 1e3,
