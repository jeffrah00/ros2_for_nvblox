...
Inference dominates the per-frame cost (typically 20–40 ms on a desktop GPU), so the absolute ceiling from the items below is ~5–10 ms/frame combined. Re-exporting the engine (FP16/INT8, smaller resolution) is the big lever; this section only covers wins that don't touch the engine.

- **Disable rate probes** (default off). Both depth nodes can create two extra Image subscriptions purely to count Hz for the FPS log; each fires a Python callback per frame that re-deserializes a full Image (1–3 ms/frame at typical rates). They are off by default — pass `enable_rate_probes:=true` only when you need the in/out Hz numbers in the log. The same data is recoverable from `ros2 topic hz <topic>`.
- **Verify shared-memory DDS transport.** Two cross-process hops (camera → depth, depth → nvblox) ride on DDS. FastDDS (the Humble default) enables shared memory for same-host comms by default; CycloneDDS does not. If SHM is off, each hop costs ~2–4 ms of serialization + loopback; with SHM it's ~0.5–1 ms.
  - Check what's in use: `ros2 doctor --report | grep -i middleware`.
  - For FastDDS, confirm "SharedMem" appears in the startup banner with `RMW_FASTRTPS_LOG_LEVEL=Info ros2 launch …`.
  - For CycloneDDS, point `CYCLONEDDS_URI` at a config that enables shared memory (Iceoryx) — see the [Cyclone DDS docs](https://cyclonedds.io/docs/cyclonedds/latest/shared_memory.html).
- **Match QoS to your nvblox config.** `use_sensor_qos:=true` switches publishers and subscribers to `BEST_EFFORT, KEEP_LAST 5`, which prevents queue-induced stalls when the downstream consumer can't keep up. Leave it off if your nvblox install uses `RELIABLE` subscribers (mismatched QoS will silently stop depth from flowing).
- **`process_every_n`** trades latency for FPS by skipping `n-1` synchronized stereo callbacks. Useful if you want a steady visualization rate at the cost of slightly stale depth.
