...
- **nvblox or RViz still showing the wrong depth** — the launch files remap `/camera0/realsense_splitter_node/output/depth` (and `..._camera_info`) onto the custom node's output. If your nvblox/RViz config subscribes to a different topic name, either edit the `SetRemap` `src=` arguments in `launch/s2m2_example.launch.py` / `launch/custom_depth_example.launch.py` to match what `ros2 node info /nvblox_node` reports, or override `s2m2_output_depth_topic` / `custom_output_depth_topic` to publish on whatever name the existing config expects.
